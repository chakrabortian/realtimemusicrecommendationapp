...
import music.recommendation.core.TestLogic.UserArtist
...
/**
  * Created by deveshkandpal on 4/15/17.
  */
object ColdStartUser {
...
//      val user3 = predictedALSModel.recommendUsers(131, 5).toList
//
//      val userSet = List(user1, user2, user3).flatten
//
//
//
//
//      val recommendations = userSet.flatMap(user => {
//        val recoUser = predictedALSModel.recommendProducts(user.user, 5).toList
//        recoUser
//      }).sortWith((rec1, rec2) => rec1.rating > rec2.rating)
//
//      val uniqueRecos = recommendations.map(r => r.product).toSet
//
//      uniqueRecos.foreach(r => {
//
//        if (!choice.contains(artistReversedZip(r.toLong)))
//          println(" user recommendations : " + artistReversedZip(r.toLong))
//
//
//      })
